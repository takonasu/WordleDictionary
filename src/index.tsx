...
const Main = () => {
  ...
  const [wordleData, setWordleData] = useState<Array<string>>([
    "now loading...",
  ]);
  ...
  const addWords = () => {
    ...
    if (blockTextInput.current && blockTextInput.current.value !== "") {
      newBlockWordList = newBlockWordList.concat(
        Array.from(blockTextInput.current.value)
      );
  ...
  const checkRule = (word: string) => {
    ...
    // 各場所から黄色文字をはじく
    let test1 = !yellowWordList.some(
      (elm) => elm.place === 0 && elm.word === oneword[0]
    );
    let test2 = !yellowWordList.some(
      (elm) => elm.place === 1 && elm.word === oneword[1]
    );
    let test3 = !yellowWordList.some(
      (elm) => elm.place === 2 && elm.word === oneword[2]
    );
    let test4 = !yellowWordList.some(
      (elm) => elm.place === 3 && elm.word === oneword[3]
    );
    let test5 = !yellowWordList.some(
      (elm) => elm.place === 4 && elm.word === oneword[4]
    );
    // 黄色文字を全てその単語は含んでいるか(場所についてはtest1-5で考慮しているのでtest6は場所を考慮しない)
    let test6 = yellowWordList
      .map((elm) => elm.word)
      .every((elm) => word.includes(elm));
          ...
          <div className="container">
            <article className="message">
              <div className="message-body">
                <p>
                  緑と黄色い文字の欄には半角英語一文字ずつ，場所に気を付けて入れてください．空欄でも構わないです．
                </p>
                <p>
                  黒い文字の欄には文字を一気に入れることができます．（一文字ずつでなくてもOK）
                </p>
                <p>
                  お問い合わせ：
